...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
      return message.safeReply("Esse comando é apenas para os donos do bot");
    }
    ...
    if (cmd.userPermissions && cmd.userPermissions?.length > 0) {
      if (!message.channel.permissionsFor(message.member).has(cmd.userPermissions)) {
        return message.safeReply(`Você precisa ${parsePermissions(cmd.userPermissions)} para este comando`);
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!message.channel.permissionsFor(message.guild.members.me).has(cmd.botPermissions)) {
        return message.safeReply(`Eu preciso ${parsePermissions(cmd.botPermissions)} para este comando`);
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return message.safeReply(
          `Você está em espera. Você pode usar novamente o comando em \`${timeformat(remaining)}\``
        );
      }
    }
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (!cmd)
      return interaction.reply({ content: "An error has occurred", flags: MessageFlags.Ephemeral }).catch(() => {});
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Esse comando é apenas para os donos do bot`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (interaction.member && cmd.userPermissions?.length > 0) {
      if (!interaction.member.permissions.has(cmd.userPermissions)) {
        return interaction.reply({
          content: `Você precisa ${parsePermissions(cmd.userPermissions)} para este comando`,
          flags: MessageFlags.Ephemeral,
        });
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!interaction.guild.members.me.permissions.has(cmd.botPermissions)) {
        return interaction.reply({
          content: `Eu preciso ${parsePermissions(cmd.botPermissions)} para este comando`,
          flags: MessageFlags.Ephemeral,
        });
      }
    }
    ...
    try {
      await interaction.deferReply({ flags: cmd.slashCommand.ephemeral && MessageFlags.Ephemeral });
