...
const coinToAdd = () => getRandomInt(10) + 1;
const xpToAdd = () => getRandomInt(19) + 1;
const coinRoles = ["1095123536202444952", "1095123576169959587"];

const levelsToUp = (level, xp) => {
  let levels = level;

  const xpNeeded = (lvl) => lvl * lvl * 100;

  while (xp >= xpNeeded(levels + 1)) {
    levels++;
  }

  return { level: levels, xp: xpNeeded(levels) };
};
...
module.exports = {
  ...
  async trackMessageStats(message, isCommand, settings) {
    const statsDb = await getMemberStats(message.guildId, message.member.id, true);
...
//    const hasCoinRole = memberRoles.some((role) => coinRoles.includes(role));

//    if (hasCoinRole) statsDb.coin += coinToAdd();
    ...
    const awarded = levelsToUp(level, xp);

    if (awarded.level > level) {
      level = awarded.level;
      xp -= awarded.xp;
  ...
  async trackVoiceStats(oldState, newState) {
    ...
    if (oldChannel && !newChannel) {
      ...
      if (voiceStates.has(member.id)) {
        ...
        const timeToAdd = Math.floor(time / 1000);

        statsDb.voice.time += timeToAdd; // add time in seconds

        // add 100 xp for every 5 minutes

        // let { xp, level } = statsDb;
        // const xpToAdd = Math.floor(timeToAdd / 300) * 100;
        // statsDb.xp += xpToAdd;
